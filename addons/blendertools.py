...
def getCamSpaceBoundingBox(ob, camera):
	...
	cam_coord = [bpy_extras.object_utils.world_to_camera_view(scene, camera, Vector(corner)) for corner in wsbb]
	return cam_coord

def getCamSpaceVertexPositions(ob, camera):
	scene = bpy.context.scene	
	cam_coord = [bpy_extras.object_utils.world_to_camera_view(scene, camera, Vector(position.co * ob.matrix_world + ob.matrix_world.translation )) for position in ob.data.vertices]
	return cam_coord
...
def getScreenSpaceBoundsOfSelection(camera, useBounds = False):
	...
	for i in bpy.context.scene.objects:
		if i.select and isinstance(i.data, bpy_types.Mesh):
